<template>
  ...
  >
    <view class="slider-range-inner" :style="{ height: height + 'px' }">
      ...
      <!-- 滑块值提示 -->
      <view v-if="tipVisible" class="range-tip" :style="lowerTipStyle">{{ format(values[0]) >= 1000?'':format(values[0])}}</view>
      <view v-if="tipVisible" class="range-tip" :style="higherTipStyle">{{ format(values[1]) > 1000?'1000以上':format(values[1]) }}</view>
    </view>
  </view>
</template>
<script>
